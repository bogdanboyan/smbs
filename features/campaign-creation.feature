...
Функционал: Создание рекламной компании
  ...
  Предыстория:
    ...
    Сценарий: Пользователь тыпается сохранить не заполненую компанию
      Допустим я захожу на /campaigns/new
      Тогда я должен увидеть "Создать компанию"
      Когда я заполню "campaign_title" значением "Просто проверяем"
      И нажму "cохранить компанию"
      Тогда я должен увидеть сообщение "Для создания компании нужно установить: Мобильный сайт, Короткий адрес и QR код"
